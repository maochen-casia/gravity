...
class SatImageAlign:
    ...
    def __call__(self,
        ...
        t_sat2world_new = torch.cat([t_left2world_init[0:2], t_sat2world[2:]], dim=0)

        pix_init = world2image(t_left2world_init[None, None, :],
                               K_sat[None],
                               R_sat2world[None],
...
class LocDataset(Dataset):
    def __init__(self, pairs, left_image_size, sat_image_size, aug=False):
        ...
        self.aug = Augmentation() if aug else None
    ...
    def __getitem__(self, index):
        ...
        K_sat = pair['K_sat']
        rotate = pair['rotate']
        t_left2world_init = pair['t_left2world_init']
        ...
        with Image.open(sat_image_path).convert('RGB') as image:
            ...
            if self.aug:
                random_offset = (torch.rand(2) * 2 - 1) * 20
                random_rotate = (torch.rand(1).item() * 2 - 1) * 180 * (np.pi / 180)
                t_left2world_align = t_left2world_init.clone()
                t_left2world_align[0:2] += random_offset
            ...
            sat_image, R_sat2world, t_sat2world = self.sat_align(image,
                                                                 ...
                                                                 rotate,
                                                                 t_left2world_align)
            sat_image = self.aug(sat_image) if self.aug else sat_image
...
def build_loc_pair_data_loaders(config, visualize=False):
    ...
    train_dataset = LocatePairDataset(data_dict['train'],
                                      left_image_size,
                                      sat_image_size,
                                      aug=True)
    val_dataset = LocatePairDataset(data_dict['val'],
                                    left_image_size,
                                    sat_image_size)
    test_dataset = LocatePairDataset(data_dict['test'],
                                     left_image_size,
                                     sat_image_size)

    num_workers = 16
    train_data_loader = DataLoader(train_dataset, batch_size, shuffle=True, num_workers=num_workers, collate_fn=train_dataset.collate_fn, pin_memory=True)
    
    val_data_loader = DataLoader(val_dataset, batch_size, shuffle=False, num_workers=num_workers, collate_fn=val_dataset.collate_fn, pin_memory=True)
    test_data_loader = DataLoader(test_dataset, batch_size, shuffle=False, num_workers=num_workers, collate_fn=test_dataset.collate_fn, pin_memory=True)
